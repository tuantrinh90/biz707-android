...
import com.halilibo.betteraudioplayer.receiver.MessageEvent;
import com.halilibo.betteraudioplayer.subtitle.CaptionsView;
import com.halilibo.betteraudioplayer.utility.EmptyCallback;
import com.halilibo.betteraudioplayer.utility.Util;
...
public class BetterAudioPlayer extends RelativeLayout implements IUserMethods,
    ...
    public BetterAudioPlayer(Context context) {
        super(context);
        init(context, null);
    }

    public BetterAudioPlayer(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public BetterAudioPlayer(Context context, AttributeSet attrs, int defStyleAttr) {
...
//    private View mToolbarFrame;
    ...
    private BetterAudioCallback mCallback;
    private BetterAudioProgressCallback mProgressCallback;
    ...
    private int mHideControlsDuration = 10000;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = context.getTheme().obtainStyledAttributes(
                    attrs,
                    R.styleable.BetterAudioPlayer,
                    0, 0);
            try {

                String source = a.getString(R.styleable.BetterAudioPlayer_bvp_source);
                if (source != null && !source.trim().isEmpty())
                    mSource = Uri.parse(source);

                String title = a.getString(R.styleable.BetterAudioPlayer_bvp_title);
                if (title != null && !title.trim().isEmpty())
                    mTitle = title;

                mPlayDrawable = a.getDrawable(
                        R.styleable.BetterAudioPlayer_bvp_playDrawable);
                mPauseDrawable = a.getDrawable(
                        R.styleable.BetterAudioPlayer_bvp_pauseDrawable);
                mRestartDrawable = a.getDrawable(
                        R.styleable.BetterAudioPlayer_bvp_restartDrawable);
                mLoadingStyle = a.getInt(
                        R.styleable.SpinKitView_SpinKit_Style, 0);
                mHideControlsDuration = a.getInteger(
                        R.styleable.BetterAudioPlayer_bvp_hideControlsDuration, mHideControlsDuration);

                mHideControlsOnPlay = a.getBoolean(
                        R.styleable.BetterAudioPlayer_bvp_hideControlsOnPlay, false);
                mAutoPlay = a.getBoolean(
                        R.styleable.BetterAudioPlayer_bvp_autoPlay, false);
                mLoop = a.getBoolean(
                        R.styleable.BetterAudioPlayer_bvp_loop, false);
                mShowTotalDuration = a.getBoolean(
                        R.styleable.BetterAudioPlayer_bvp_showTotalDuration, false);
                mBottomProgressBarVisibility = a.getBoolean(
                        R.styleable.BetterAudioPlayer_bvp_showBottomProgressBar, false);
                mSwipeEnabled = a.getBoolean(
                        R.styleable.BetterAudioPlayer_bvp_swipeGesturesEnabled, false);
                mShowToolbar = a.getBoolean(
                        R.styleable.BetterAudioPlayer_bvp_showToolbar, true);
                mControlsDisabled = a.getBoolean(
                        R.styleable.BetterAudioPlayer_bvp_disableControls, false);
                mSubViewTextSize = a.getDimensionPixelSize(
                        R.styleable.BetterAudioPlayer_bvp_captionSize,
                        getResources().getDimensionPixelSize(R.dimen.bvp_subtitle_size));
                mSubViewTextColor = a.getColor(
                        R.styleable.BetterAudioPlayer_bvp_captionColor,
        ...
        if (mPlayDrawable == null)
            mPlayDrawable = ContextCompat.getDrawable(context, R.drawable.ic_play);
        if (mPauseDrawable == null)
            mPauseDrawable = ContextCompat.getDrawable(context, R.drawable.ic_pause);
    ...
    @Override
    public void setCallback(@NonNull BetterAudioCallback callback) {
        mCallback = callback;
    }
    ...
    @Override
    public void setProgressCallback(@NonNull BetterAudioProgressCallback callback) {
        mProgressCallback = callback;
    }

    @Override
    public void setButtonDrawable(@BetterAudioPlayer.LoadingStyle int type,
...
//    }
    ...
    @Override
    public void setImgContent(String url) {
        RequestOptions options = new RequestOptions()
                .fitCenter()
                .error(R.drawable.ic_img_book_default)
                .diskCacheStrategy(DiskCacheStrategy.ALL)
                .priority(Priority.HIGH);
        Glide.with(getContext()).applyDefaultRequestOptions(options).load(url).into(mImgContent);
    }

    @Override
    public void setBackground() {
        mTextureFrame.setBackground(getResources().getDrawable(R.drawable.bg_video));
    }
    ...
    public void toggleControls() {
        ...
        if (isControlsShown()) {
            ...
            if (mHideControlsDuration >= 0 && mHandler != null) {
...
//        if (isFullScreen) {
//            adjustAspectRatio(1920, height, mPlayer.getVideoWidth(), mPlayer.getVideoHeight());
//        } else {
...
//        }
    ...
    protected void onFinishInflate() {
        ...
        mContentFrame = li.inflate(R.layout.bvp_include_content_view, this, false);
        LayoutParams mContentLp = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.MATCH_PARENT);
        mContentLp.addRule(RelativeLayout.CENTER_IN_PARENT);
        addView(mContentFrame, mContentLp);
        mImgContent = mContentFrame.findViewById(R.id.imgContent);
        llImageContent = mContentFrame.findViewById(R.id.llImageContent);

        // Inflate controls
        mControlsFrame = li.inflate(R.layout.bvp_include_controls_new, this, false);
...
//        mLabelDuration.setOnClickListener(this);
        ...
        mBtnSubtitle = (ImageButton) mControlsFrame.findViewById(R.id.btnSub);
        mBtnSubtitle.setOnClickListener(this);

        mBtnReplay = (ImageButton) mControlsFrame.findViewById(R.id.btnReplay);
        ...
        mBtnForward = (ImageButton) mControlsFrame.findViewById(R.id.btnForward);
        mBtnForward.setOnClickListener(this);

        mBtnBackward = (ImageButton) mControlsFrame.findViewById(R.id.btnbackward);
...
//                mSubView.setVisibility(INVISIBLE);
//                mBtnSubtitle.setImageDrawable(getResources().getDrawable(R.drawable.ic_sub));
//            } else {
//                isShowSubtitle = true;
//                mSubView.setVisibility(VISIBLE);
//                mBtnSubtitle.setImageDrawable(getResources().getDrawable(R.drawable.ic_sub_active));
//            }
        } else if (id == R.id.btnReplay) {
            if (isReplay) {
                isReplay = false;
                this.setLoop(false);
                mBtnReplay.setImageDrawable(getResources().getDrawable(R.drawable.ic_replay));
            } else {
                isReplay = true;
                this.setLoop(true);
                mBtnReplay.setImageDrawable(getResources().getDrawable(R.drawable.ic_replay_active));
            }
